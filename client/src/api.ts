...
export const api = {
  ...
  heartbeat:(args:{roomId:number, participantId:number, token?:string}):Promise<{alive:boolean, removed:number}> => fetchJson('/api/heartbeat', {method:'POST', body:JSON.stringify(args)}),
  sendChar:(args:{roomId:number, participantId:number, token?:string, char:string, seq?:number}):Promise<{content:string, lineIdx:number, position:number, participantId:number, buffered?:boolean, duplicate?:boolean, expected?:number}> => fetchJson('/api/char', {method:'POST', body:JSON.stringify(args)}),
  sendBackspace:(args:{roomId:number, participantId:number, token?:string, seq?:number}):Promise<{content:string, lineIdx:number, participantId:number, buffered?:boolean, duplicate?:boolean}> => fetchJson('/api/backspace', {method:'POST', body:JSON.stringify(args)}),
  commitLine:(args:{roomId:number, participantId:number, token?:string, seq?:number}):Promise<{newLineIdx:number, committedContent:string, committedAt:number, buffered?:boolean, duplicate?:boolean}> => fetchJson('/api/commit', {method:'POST', body:JSON.stringify(args)}),
