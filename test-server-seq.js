...
async function createRoomAndJoin(handle='tester'){
  ...
  return {roomId, participantId: joinJ.participant.id, token: joinJ.participant.token};
}
...
describe('Seq ordering - x then Backspace reordered', () => {
  it('typing x then Backspace with reordered requests still results in empty', async () => {
    const {roomId, participantId, token} = await createRoomAndJoin('seqtest1');
    // Simulate client seq: 1=x, 2=Backspace, but server receives 2 before 1
    // Send Backspace seq2 first (out-of-order, should buffer)
    const {res: r2, json: j2} = await fetchJson('/api/backspace', {method:'POST', body:JSON.stringify({roomId, participantId, token, seq:2})});
    ...
    const {res: r1, json: j1} = await fetchJson('/api/char', {method:'POST', body:JSON.stringify({roomId, participantId, token, char:'x', seq:1})});
  ...
  it('duplicate seq ignored', async () => {
    const {roomId, participantId, token} = await createRoomAndJoin('dup');
    const {res: r1} = await fetchJson('/api/char', {method:'POST', body:JSON.stringify({roomId, participantId, token, char:'a', seq:1})});
    assert.equal(r1.ok, true);
    const {json: state1} = await fetchJson(`/api/room-state?roomId=${roomId}`);
    assert.equal(state1.participants[0].activeContent, 'a');

    const {res: rDup, json: jDup} = await fetchJson('/api/char', {method:'POST', body:JSON.stringify({roomId, participantId, token, char:'a', seq:1})});
  ...
  it('out-of-order buffering preserves order for multiple chars', async () => {
    const {roomId, participantId, token} = await createRoomAndJoin('ooo');
    // Send seq3, seq2 first (both buffered)
    await fetchJson('/api/char', {method:'POST', body:JSON.stringify({roomId, participantId, token, char:'c', seq:3})});
    await fetchJson('/api/char', {method:'POST', body:JSON.stringify({roomId, participantId, token, char:'b', seq:2})});
    ...
    await fetchJson('/api/char', {method:'POST', body:JSON.stringify({roomId, participantId, token, char:'a', seq:1})});
...
describe('A Enter B sequence', () => {
  it('commit AB together should not happen, B in new active line', async () => {
    const {roomId, participantId, token} = await createRoomAndJoin('aeb');
    // Type A seq1
    await fetchJson('/api/char', {method:'POST', body:JSON.stringify({roomId, participantId, token, char:'A', seq:1})});
    // Commit seq2 (commits A)
    const {json: commitJ} = await fetchJson('/api/commit', {method:'POST', body:JSON.stringify({roomId, participantId, token, seq:2})});
    assert.equal(commitJ.committedContent, 'A');

    // Type B seq3 after commit (should be in new active line, not committed together)
    await fetchJson('/api/char', {method:'POST', body:JSON.stringify({roomId, participantId, token, char:'B', seq:3})});
  ...
  it('A Enter B with out-of-order arrival still orders correctly', async () => {
    const {roomId, participantId, token} = await createRoomAndJoin('aeb2');
    // Send commit seq2 before char seq1 (out-of-order)
    const {res: r2} = await fetchJson('/api/commit', {method:'POST', body:JSON.stringify({roomId, participantId, token, seq:2})});
...
describe('Commit empty allowed', () => {
  it('empty commit with seq still works', async () => {
    const {roomId, participantId, token} = await createRoomAndJoin('emptyseq');
    const {res} = await fetchJson('/api/commit', {method:'POST', body:JSON.stringify({roomId, participantId, token, seq:1})});
