...
Done and closed tasks, moved out of [TODO.md](../TODO.md) on 2026-09-10 with
their text as it stood when they were finished. Task numbers are stable and
never reused: the next task takes the number after the highest one in either
file. A task is done when its checkbox is ticked and the change is on master;
tasks 8, 9, and 20 are closed, decided not to do, with the reason in their
first bullet. The open tasks and their order stay in TODO.md.

The first twelve tasks came from the initial code review; the pending-commit
matching finding (item 5 in that review) was intentionally excluded. Tasks 10
and 11 recorded the approved direction for the rewrite, server echo and the
WebSocket chat transport, and superseded preserving optimistic rendering and
repairing the HTTP character stream.
...
- **Protocol docs:** Update [PROTOCOL.md](PROTOCOL.md) with token issuance,
...
- **Protocol docs:** Update [PROTOCOL.md](PROTOCOL.md) with implemented
...
- **Protocol docs:** Update [PROTOCOL.md](PROTOCOL.md) to distinguish
...
- **Protocol docs:** Update [PROTOCOL.md](PROTOCOL.md) with resulting
...
- **Protocol docs:** Update [PROTOCOL.md](PROTOCOL.md) with the socket URL/path,
...
- **Related work:** Coordinate with task 11; this task owns display semantics,
  ...
  [UX](USER_EXPERIENCE.md), [design](DESIGN.md), [README](../README.md),
...
- **Protocol docs:** Rewrite client interpretation, commands, timing, and example
  exchanges in [PROTOCOL.md](PROTOCOL.md) for server echo. Document new server
...
- **Related work:** Implement in coordination with task 10. Reassess issue 2's
  ...
  Update [design](DESIGN.md), [README](../README.md), and
  [agent guidance](../AGENTS.md) to document the implemented transport contract.
- **Protocol docs:** Update [PROTOCOL.md](PROTOCOL.md) alongside migration:
...
- **Protocol docs:** Add both messages, validation, and the no-persistence
  rule to [PROTOCOL.md](PROTOCOL.md); describe the feature in
  [USER_EXPERIENCE.md](USER_EXPERIENCE.md).
...
- **Protocol docs:** none. Note the one-gesture requirement in
  [USER_EXPERIENCE.md](USER_EXPERIENCE.md) next to the chirp sentence.
...
- [x] **Review finding, joining** (done: one word of letters, marks, digits and `_`, checked by the same rule in the lobby and in `/api/join`; emoji stay rejected pending Alex's call)
